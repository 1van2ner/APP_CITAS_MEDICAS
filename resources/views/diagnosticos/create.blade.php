...
<div class="min-h-screen bg-gradient-to-br from-amber-50 to-slate-100 py-8 px-4">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-amber-900">Crear Diagnóstico</h1>
        </div>

        <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-8">
            <form action="{{ route('diagnosticos.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" required rows="4" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">{{ old('descripcion') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <input type="datetime-local" name="fecha" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" value="{{ old('fecha') }}">
                            @error('fecha') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Gravedad</label>
                            <input type="text" name="gravedad" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" value="{{ old('gravedad') }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="tipo_diagnostico" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent" value="{{ old('tipo_diagnostico') }}">
                    ...
                    <div>
                        ...
                        <textarea name="recomendaciones" rows="3" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">{{ old('recomendaciones') }}</textarea>
